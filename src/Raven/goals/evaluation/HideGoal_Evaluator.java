...
public class HideGoal_Evaluator extends Goal_Evaluator {
    ...
    public HideGoal_Evaluator(double bias) {
        super(bias);
    }
    ...
    public double CalculateDesirability(Raven_Bot pBot) {
        ...
        if (pBot.GetTargetSys().isTargetPresent() && pBot.GetTargetSys().isTargetShootable() && pBot.GetTargetBot().GetTargetBot() == pBot) {
            final double Tweaker = 0.5;
